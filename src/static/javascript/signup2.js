...
//for all images
Array.from(document.getElementsByClassName("signup2-buttons")).forEach(function(element) {
...
function colorBlindMode(enabled) {
    const signupWhite = document.getElementById("signup2-white");
  
    if (enabled) {
        // Apply colorblind filter to each signup2-buttons element
        Array.from(document.getElementsByClassName("signup2-buttons")).forEach(function (element) {
            element.style.filter = "brightness(150%) saturate(80%) hue-rotate(-30deg)";
        });
  
        // Set light gray background color for #signup2-white element
        signupWhite.style.backgroundColor = "#D3D3D3";
  
        console.log("Colorblind mode enabled");
    } else {
        // Remove colorblind filter from each signup2-buttons element
        Array.from(document.getElementsByClassName("signup2-buttons")).forEach(function (element) {
            element.style.filter = "";
        });
  
        // Restore white background color for #signup2-white element
